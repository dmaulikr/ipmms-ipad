...
- (id)initWithFrame:(CGRect)frame forOrientation:(UIInterfaceOrientation) p_intOrientation andReturnMethod:(METHODCALLBACK) p_returnMethod andNaviCallbacks:(METHODCALLBACK) p_naviButtonCallbacks
{
    ...
    if (self) {
        ...
        _webdataName= [[NSString alloc] initWithFormat:@"%@",@"FEESLIST"];
        //_proxynotification = [[NSString alloc] initWithFormat:@"%@",p_proxynotificationname];
        _returnMethod = p_returnMethod;
        _cacheName = [[NSString alloc] initWithString:@"ALLFEES"];
        _naviButtonsCallbacks = p_naviButtonCallbacks;
        ...
        NSDictionary *naviInfo = [[NSDictionary alloc] initWithObjectsAndKeys:buttonData,[NSString stringWithFormat:@"%d",101] , @"Select a Fees",@"navititle" ,nil];
        _naviButtonsCallbacks(naviInfo);
        [self generateData];
    }
    return self;
}
...
- (void) generateData
{
    if (populationOnProgress==NO)
    {
        ...
        METHODCALLBACK l_feesDataGenMethod = ^(NSDictionary *p_dictInfo)
        {
            [self feesDataGenerated:p_dictInfo];
        };
        [[gymWSProxy alloc] initWithReportType:_webdataName andInputParams:nil andReturnMethod:l_feesDataGenMethod];
    }    
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [returnInfo setValue:[NSString stringWithString:@"FeesSelected"] forKey:@"notify"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell == nil) 
    {
        ...
        lblFees = [[UILabel alloc] initWithFrame:CGRectMake(1, 1, 623, 34)];
        lblFees.font = [UIFont boldSystemFontOfSize:18.0f];
        [lblFees setBackgroundColor:[UIColor colorWithRed:205.0f/255.0f green:133.0f/255.0f blue:63.0f/255.0f alpha:1.0]];
        lblFees.tag = 1;
        [cell.contentView addSubview:lblFees];
    }
    fees = [[NSString alloc] initWithFormat:@"  %@",[tmpDict valueForKey:@"FEENAME"]];
    lblFees = (UILabel*) [cell.contentView viewWithTag:1];
    lblFees.text = fees;
    return cell;
}
...
{
    NSMutableDictionary *returnInfo = [[NSMutableDictionary alloc] init];
    [returnInfo setValue:[NSString stringWithString:@"FeesSelectCancel"] forKey:@"notify"];
